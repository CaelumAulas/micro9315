...
docker/images:
	@for f in $(projects); do cd $${f}; docker build . -t $(MAINTAINER)/$${f}-$(PREFIX) && cd .. ; done
...
docker/images/push:
	@for f in $(projects); do cd $${f}; docker push $(MAINTAINER)/$${f}-$(PREFIX) && cd .. ; done

docker/images/pull:
	@for f in $(projects); do cd $${f}; docker pull $(MAINTAINER)/$${f}-$(PREFIX) && cd .. ; done
